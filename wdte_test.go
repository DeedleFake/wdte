...
func TestBasics(t *testing.T) {
	...
	runTests(t, []test{
		...
		{
			name:   "Simple/Closure",
			script: `let q => 1; let add n => + n q; let q => 2; add 1;`,
			ret:    wdte.Number(2),
		},
		{
			name:   "Simple/Closure/Partial",
			script: `let q => 1; let add n m => + n m q; let q => 2; (add 3) 5;`,
			ret:    wdte.Number(9),
		},
		{
		...
		{
			name:   "ScopeCaching/Lambda",
			script: `let io => import 'io'; let test => import 'test'; let impure x => (@ x => test.reader -> io.string); [x; x];`,
			ret:    wdte.Array{wdte.String("one"), wdte.String("two")},
			im: wdte.ImportFunc(func(im string) (*wdte.Scope, error) {
				if im == "test" {
					return wdte.S().Map(map[wdte.ID]wdte.Func{
						"reader": wdteio.Reader{&pieceReader{
